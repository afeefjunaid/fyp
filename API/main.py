...
# # Define classifier model
# class CLIPSkinClassifier(nn.Module):
#     def _init_(self, clip_model, num_classes=3):
#         super(CLIPSkinClassifier, self)._init_()
#         self.encoder = clip_model.visual
#         self.classifier = nn.Sequential(
#             nn.Linear(self.encoder.output_dim, 256),
#             nn.ReLU(),
...
@app.post("/predict")
async def predict(file: UploadFile = File(...)):  # Changed from File(None) to File(...) to make it required
    try:
        # Step 1: Read the uploaded file
        image_data = await file.read()
        image = read_file_as_image(image_data)
        ...
        return {
            "predicted_class": predicted_class,  # Changed from "class" to "predicted_class" for consistency
            "confidence_score": confidence,      # Changed from "confidence" to "confidence_score" for consistency
            "probabilities": {
                name: float(prob)
                for name, prob in zip(CLASS_NAMES, pred_array[0])
            }
...
if __name__ == "__main__":
    uvicorn.run(app, host="0.0.0.0", port=8080)  # Changed from "localhost" to "0.0.0.0" to allow external connections